...
public class FilmeController : ControllerBase
{
    ...
    [HttpPatch("{id}")]
    public IActionResult AtualizaFilmeCaracteristica(int id, JsonPatchDocument<UpdateFilmeDto> patch)
    {
        var filme = _context.Filmes.FirstOrDefault(filme => filme.Id == id);
        if (filme == null) return NotFound();
        ...
        if (!TryValidateModel(filmeAtualizar)) return ValidationProblem();

        _mapper.Map(RetornaFilmePorId, filme);
        _context.SaveChanges();
        return NoContent();
